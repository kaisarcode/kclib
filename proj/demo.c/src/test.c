/**
 * test.c - libdemo public API and CLI contract tests.
...
#ifndef KC_DEMO_TEST_CLI
#define KC_DEMO_TEST_CLI ""
#endif
...
/**
 * Test kc_demo_greet and kc_demo_free.
 * @return 0 on success, 1 on failure.
 */
...
/**
 * Run the demo CLI and capture stdout, stderr, and exit status.
...
static int test_cli_run(
    ...
    if (test_to_wide(KC_DEMO_TEST_CLI, exe, MAX_PATH)) return 1;
...
/**
 * Test the complete demo CLI contract as one grouped case.
 * @return 0 on success, 1 on failure.
 */
static int case_kc_demo_cli(void) {
    const char *name = "kc_demo_cli";
    const char *detail = "default/name greeting, help, version, and diagnostics";
    char out[4096];
    char err[4096];
    int status = 0;
    int fail = 0;
    char *plain[] = { (char *)KC_DEMO_TEST_CLI, NULL };
    char *short_name[] = {
        (char *)KC_DEMO_TEST_CLI, "-n", "John", NULL
    };
    char *long_name[] = {
        (char *)KC_DEMO_TEST_CLI, "--name", "Jane", NULL
    };
    char *help[] = { (char *)KC_DEMO_TEST_CLI, "-h", NULL };
    char *long_help[] = { (char *)KC_DEMO_TEST_CLI, "--help", NULL };
    char *version[] = { (char *)KC_DEMO_TEST_CLI, "-v", NULL };
    char *long_version[] = { (char *)KC_DEMO_TEST_CLI, "--version", NULL };
    char *missing_name[] = { (char *)KC_DEMO_TEST_CLI, "-n", NULL };
    char *unknown[] = { (char *)KC_DEMO_TEST_CLI, "--nope", NULL };

    if (KC_DEMO_TEST_CLI[0] == '\0') {
    ...
    fail += expect_true("CLI -v prints build", strstr(out, "demo build") != NULL);
...
static int case_all(void) {
    ...
    test_case_total = 2;
#else
    int cli_enabled = KC_DEMO_TEST_CLI[0] != '\0';
...
/**
 * Run one libdemo contract test case.
...
#ifndef __EMSCRIPTEN__
    if (strcmp(argv[1], "kc_demo_cli") == 0) return case_kc_demo_cli();
#endif
